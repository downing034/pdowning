...
describe('Portfolio', () => {
  it('renders Portfolio', () => {
    ...
    expect(rocketLeague).toHaveDisplayName('ComingSoonPortfolioItem')
    ...
    expect(bachelorette).toHaveProp('image', 'bachelorette-jeopardy.jpg')

    let emptyProfileItem1 = secondCodeOnlyRow.childAt(1)
    expect(emptyProfileItem1).toHaveDisplayName('EmptyPortfolioItem')

    let emptyProfileItem2 = secondCodeOnlyRow.childAt(2)
    expect(emptyProfileItem2).toHaveDisplayName('EmptyPortfolioItem')
  });
});
